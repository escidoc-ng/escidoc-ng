...
/**
 * Spring-security JavaConfig class defining the security context of the escidocng repository
 */
@Configuration
@EnableWebMvcSecurity
@EnableGlobalMethodSecurity(prePostEnabled = true)
@EnableAspectJAutoProxy
public class EscidocngServerSecurityConfiguration extends WebSecurityConfigurerAdapter {

    @Autowired
    private Environment env;

    @Autowired
    @Qualifier("escidocngElasticSearchAuthenticationManager")
    private AuthenticationManager authenticationManager;

    @Autowired
    @Qualifier("escidocngOpenIdAuthenticationProvider")
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.csrf().requireCsrfProtectionMatcher(new EscidocngCsrfRequestMatcher());
        if (!Boolean.valueOf(env.getProperty("escidocng.security.csrf.enabled", "true"))) {
            http.csrf().disable();
        }
    }
    ...
    private void configureProxy() {
        if (StringUtils.isNotBlank(env.getProperty("escidocng.proxy.name"))) {
            ...
            proxyProps.setProxyHostName(env.getProperty("escidocng.proxy.name"));
            if (StringUtils.isNotBlank(env.getProperty("escidocng.proxy.port"))) {
                proxyProps.setProxyPort(Integer.parseInt(env.getProperty("escidocng.proxy.port")));
            }
            HttpClientFactory.setProxyProperties(proxyProps);
        }
    }

    private class EscidocngCsrfRequestMatcher implements RequestMatcher {
