...
/**
 * Client implementation for the Escidocng server
 */
public class EscidocngClient {

    private static final Logger log = LoggerFactory.getLogger(EscidocngClient.class);
    ...
    public EscidocngClient(URI escidocngUri, String username, String password) {
        if (escidocngUri.toASCIIString().endsWith("/")) {
            this.escidocngUri = URI.create(escidocngUri.toASCIIString().substring(0, escidocngUri.toASCIIString().length() -1));
        }else {
            this.escidocngUri = escidocngUri;
        }
        final HttpHost escidocng = new HttpHost(escidocngUri.getHost(), escidocngUri.getPort());
        this.executor = new ThreadLocal<Executor>() {
            @Override
            protected Executor initialValue() {
                return Executor.newInstance()
                        .auth(escidocng, username, password)
                        .authPreemptive(escidocng);
            }
        };
    }
    ...
    public Object[] executeAsUser(String method, String uri, Object body, String username,
        ...
        HttpUriRequest request = getRequest(method, this.escidocngUri + uri, body);
        if (request != null) {
    ...
    /**
     * Retrieve a {@link de.escidocng.model.state.EscidocngState} response from the repository containing
     * detailed state information
     * 
     * @return a EscidocngState POJO
     * @throws IOException
     */
    public EscidocngState retrieveState() throws IOException {
        final HttpResponse resp = this.execute(Request.Get(escidocngUri + "/state")
                .useExpectContinue())
                .returnResponse();
        if (resp.getStatusLine().getStatusCode() != 200) {
            throw new IOException("Unable to retrieve EscidocngState response from the repository");
        }
        return mapper.readValue(resp.getEntity().getContent(), EscidocngState.class);
    }

    /**
     * Retrieve a {@link de.escidocng.model.Describe} response from the repository containing general state
     * information
     * 
    ...
    public Describe retrieveDescribe() throws IOException {
        final HttpResponse resp = this.execute(Request.Get(escidocngUri + "/describe")
                .useExpectContinue())
                .returnResponse();
        if (resp.getStatusLine().getStatusCode() != 200) {
    ...
    /**
     * Retrieve a {@link de.escidocng.model.Metadata} of an Entity from the repository
     * 
    ...
    public Metadata retrieveMetadata(String entityId, String metadataName) throws IOException {
        final HttpResponse resp =
                this.execute(Request.Get(escidocngUri + "/entity/" + entityId + "/metadata/" + metadataName)
                        .useExpectContinue())
                        .returnResponse();
        if (resp.getStatusLine().getStatusCode() != 200) {
    ...
    public InputStream retrieveMetadataContent(String entityId, String metadataName) throws IOException {
        final HttpResponse resp =
                this.execute(Request.Get(escidocngUri + "/entity/" + entityId + "/metadata/" + metadataName + "/content")
                        .useExpectContinue())
                        .returnResponse();
        if (resp.getStatusLine().getStatusCode() != 200) {
            log.error("Unable to fetch meta data\n{}", EntityUtils.toString(resp.getEntity()));
    ...
    /**
     * Retrieve a {@link de.escidocng.model.Metadata} of a Binary from the repository
     * 
     * @param entityId the entity's id
     * @param metadataName the meta data set's name
     * @return the Metadata as a POJO
    ...
    public Metadata retrieveBinaryMetadata(String entityId, String binaryName, String metadataName)
        ...
        final HttpResponse resp =
                this.execute(
                        Request.Get(
                                escidocngUri + "/entity/" + entityId + "/binary/" + binaryName + "/metadata/" +
                                        metadataName)
                                .useExpectContinue())
                        .returnResponse();
        if (resp.getStatusLine().getStatusCode() != 200) {
    ...
    /**
     * Add {@link de.escidocng.model.Metadata} to an existing entity
     * 
     * @param entityId the entity's id
    ...
    public void postMetadata(String entityId, Metadata md) throws IOException {
        final HttpResponse resp = this.execute(Request.Post(escidocngUri + "/entity/" + entityId + "/metadata")
                .useExpectContinue()
    ...
    public void postMetadataMultipart(String entityId, Metadata md) throws IOException {
        HttpResponse resp =
                this.execute(
                        Request.Post(escidocngUri + "/entity/" + entityId + "/metadata").body(MultipartEntityBuilder.create()
    ...
    /**
     * Add {@link de.escidocng.model.Metadata} to an existing binary
     * 
     * @param entityId the entity's id
     * @param md the Metadata object
     * @throws IOException
    ...
    public void postBinaryMetadata(String entityId, String binaryName, Metadata md) throws IOException {
        final HttpResponse resp =
                this.execute(Request.Post(escidocngUri + "/entity/" + entityId + "/binary/" + binaryName + "/metadata")
                        .useExpectContinue()
                        .bodyString(mapper.writeValueAsString(md), ContentType.APPLICATION_JSON))
                        .returnResponse();
        if (resp.getStatusLine().getStatusCode() != 201) {
    ...
    public void postBinaryMetadataMultipart(String entityId, String binaryName, Metadata md) throws IOException {
        HttpResponse resp =
                this.execute(
                        Request.Post(escidocngUri + "/entity/" + entityId + "/binary/" + binaryName + "/metadata").body(MultipartEntityBuilder.create()
                                .addTextBody("name", md.getName())
                                .addTextBody("type", md.getType())
                                .addTextBody("indexInline", new Boolean(md.isIndexInline()).toString())
                                .addBinaryBody(
    ...
    /**
     * Delete the {@link de.escidocng.model.Metadata} of a {@link de.escidocng.model.Entity}
     * 
     * @param entityId the entity's id
     * @param metadataName the meta data set's name
     * @throws IOException
    ...
    public void deleteMetadata(String entityId, String metadataName) throws IOException {
        final HttpResponse resp =
                this.execute(Request.Delete(escidocngUri + "/entity/" + entityId + "/metadata/" + metadataName)
                        .useExpectContinue())
                        .returnResponse();
        if (resp.getStatusLine().getStatusCode() != 200) {
    ...
    /**
     * Delete the {@link de.escidocng.model.Metadata} of a {@link de.escidocng.model.Binary}
     * 
     * @param entityId the entity's id
    ...
    /**
     * Retrieve {@link de.escidocng.model.Binary} from the repository
     * 
     * @param entityId the entity's id
     * @param binaryName the binary's name
    ...
    public Binary retrieveBinary(String entityId, String binaryName) throws IOException {
        final HttpResponse resp =
                this.execute(Request.Get(escidocngUri + "/entity/" + entityId + "/binary/" + binaryName)
                        .useExpectContinue())
                        .returnResponse();
        if (resp.getStatusLine().getStatusCode() != 200) {
    ...
    /**
     * Add a {@link de.escidocng.model.Binary} to an existing entity
     * 
     * @param entityId the entity's id
    ...
    public void postBinary(String entityId, Binary bin) throws IOException {
        final HttpResponse resp = this.execute(Request.Post(escidocngUri + "/entity/" + entityId + "/binary")
                .useExpectContinue()
    ...
    /**
     * Delete a {@link de.escidocng.model.Binary} in the repository
     * 
     * @param entityId the entity's id
     * @param binaryName the binary's name
     * @throws IOException
    ...
    public void deleteBinary(String entityId, String binaryName) throws IOException {
        final HttpResponse resp =
                this.execute(Request.Delete(escidocngUri + "/entity/" + entityId + "/binary/" + binaryName)
                        .useExpectContinue())
                        .returnResponse();
        if (resp.getStatusLine().getStatusCode() != 200) {
    ...
    public InputStream retrieveBinaryContent(String entityId, String binaryName) throws IOException {
        final HttpResponse resp =
                this.execute(Request.Get(escidocngUri + "/entity/" + entityId + "/binary/" + binaryName + "/content")
                        .useExpectContinue())
                        .returnResponse();
        if (resp.getStatusLine().getStatusCode() != 200) {
    ...
    /**
     * Update an {@link de.escidocng.model.Entity} in the escidocng repository
     * 
    ...
    public void updateEntity(Entity e) throws IOException {
        ...
        final HttpResponse resp = this.execute(Request.Put(escidocngUri + "/entity/" + e.getId())
                .useExpectContinue()
    ...
    public void updateEntity(String entityId, String entity) throws IOException {
        ...
        final HttpResponse resp = this.execute(Request.Put(escidocngUri + "/entity/" + entityId)
                .useExpectContinue()
    ...
    /**
     * Delete an entity in the escidocng repository
     * 
     * @param id the id of the entity to delete
     * @throws IOException
     */
    public void deleteEntity(String id) throws IOException {
        final HttpResponse resp = this.execute(Request.Delete(escidocngUri + "/entity/" + id)
                .useExpectContinue())
                .returnResponse();
        if (resp.getStatusLine().getStatusCode() != 200) {
    ...
    /**
     * Post an {@link de.escidocng.model.Entity} to the Escidocng server
     * 
    ...
    public String postEntity(Entity e) throws IOException {
        final HttpResponse resp = this.execute(Request.Post(escidocngUri + "/entity")
                .useExpectContinue()
                .bodyString(mapper.writeValueAsString(e), ContentType.APPLICATION_JSON))
                .returnResponse();
        if (resp.getStatusLine().getStatusCode() != 201) {
            log.error("Unable to post entity to Escidocng at {}\n{}", escidocngUri, EntityUtils.toString(resp.getEntity()));
    ...
    public String postEntityMultipart(Entity entity) throws IOException {
        ...
        HttpResponse resp =
                this.execute(
                        Request.Post(escidocngUri + "/entity/").body(entityBuilder.build())).returnResponse();
...
//                } catch (InterruptedException e1) {}
    ...
    /**
     * Retrieve an entity using a HTTP GET from the Escidocng server
     * 
     * @param id the Id of the entity
     * @return An entity object
     */
    public Entity retrieveEntity(String id) throws IOException {
        final HttpResponse resp = this.execute(Request.Get(escidocngUri + "/entity/" + id)
                .useExpectContinue()
    ...
    public String createUser(String name, String password, List<String> level2Ids) throws IOException {
        ...
        HttpResponse resp =
                this.execute(
                Request.Post(escidocngUri + "/user")
        ...
        resp =
                this.execute(
                Request.Post(escidocngUri + "/confirm/" + token)
                        .body(MultipartEntityBuilder.create()
    ...
    public String createRoles(String username, List<Role> roles) throws IOException {
        ...
        final HttpResponse resp = this.execute(Request.Post(escidocngUri + "/user/" + username + "/roles")
                .useExpectContinue()
